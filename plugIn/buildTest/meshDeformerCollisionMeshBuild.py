...
nodeType       = 'meshDeformerCollisionMesh'  
releaseFolder = ['Debug','Release']
pathNodeCpp    = 'D:/mcantat_BDD/projects/code/maya/c++/mayaNode/meshDeformerCollisionMesh/Build/{}/meshDeformerCollisionMesh.mll'.format( releaseFolder[releaseMode] )
pathNode   = [ pathNodePython , pathNodeCpp ][useNodeCpp]
#__________________________ BUILD TEST INFO
lodSuffixs = [ 'Low' , 'High' ]
pathBuildTest = 'D:/mcantat_BDD/projects/nodeTest/meshDeformerCollisionMeshBuildTest{}.ma'.format(lodSuffixs[highPoly])
...
mc.connectAttr( ( cageShape      + '.worldMesh[0]' )  , ( newNode + '.meshCollide'     ) )
mc.connectAttr( ( 'inMatrix'     + '.worldMatrix[0]' )  , ( newNode + '.inSource'     ) )
...
mc.setAttr( newNode + '.sampleMode_r', 1)
mc.setAttr( newNode + '.rayDistPosMax_r', 0)
mc.setAttr( newNode + '.rayOutputSortMode_r', 3)
