...
path = 'D:\mcantat_BDD\projects\code\maya\python\plugIn\annotateCamera.py'
...
class annotateCamera(ommpx.MPxLocatorNode):

    kPluginNodeTypeName = 'annotateCamera'
...
def initializePlugin(obj):
    ...
    try:
        plugin.registerNode( annotateCamera.kPluginNodeTypeName, annotateCamera.kPluginNodeId, annotateCamera.nodeCreator, annotateCamera.nodeInitializer, ommpx.MPxNode.kLocatorNode)
    except:
        raise Exception('Failed to register node: {0}'.format(annotateCamera.kPluginNodeTypeName) )
...
def uninitializePlugin(obj):
    ...
    try:
        plugin.deregisterNode( annotateCamera.kPluginNodeId )
    except:
        raise Exception('Failed to unregister node: {0}'.format(annotateCamera.kPluginNodeTypeName) )
...
class annotateCameraDrawOverride(omr.MPxDrawOverride):


    def __init__(self):
        omr.MPxDrawOverride.__init__(self)   

    def prepareForDraw(objPath, cameraPath, frameContext, oldData):
        DEBUG_C = 1

        if (DEBUG_C): print("glDrawDrawOverride________________________________________PREPARE - GET FROM MAYA")
        #DISPLAY
        MFloatMatrix      camMatrix          = dagPath_getAttrMFloatMatrix(            objPath, glDraw::inAttrs[1])
        int               drawPlaneMode      = dagPath_getAttrInt(                     objPath, glDraw::inAttrs[2])
        #OBJ INFO
        vector<int>       shapes             = dagPath_getAttrIntArray(                objPath, glDraw::inAttrs[4]) 
        MFloatVectorArray coords             = dagPath_getAttrMFloatVectorArray(       objPath, glDraw::inAttrs[5])
        MFloatVectorArray screenSpaceOffsets = dagPath_getAttrMFloatVectorArray(       objPath, glDraw::inAttrs[6])
        vector<float>     sizes              = dagPath_getAttrFloatArray(              objPath, glDraw::inAttrs[7])
        vector<float>     thicknesses        = dagPath_getAttrFloatArray(              objPath, glDraw::inAttrs[8])
        MFloatPointArray  colors             = dagPath_getAttrMFloatPointArray(        objPath, glDraw::inAttrs[9])
        vector<MFloatPointArray>  pColors(0);
        vector<MString>   names              = dagPath_getAttrMStringArray(            objPath, glDraw::inAttrs[10])
        '''
    ...
    if (DEBUG_C) print_string("glDrawDrawOverride________________________________________ CREATE DATA");

    glDrawData* data = dynamic_cast<glDrawData*>(oldData);
    if (!data) { data = new glDrawData(); }

    if (DEBUG_C) print_string("glDrawDrawOverride________________________________________PREPARE - FILL DATA ATTR");
    ...
    data->names = names;
    data->fills = fills;
    data->spaces = spaces; // screen , world
    data->maxMemorys = maxMemorys;
    data->projPlanes = projPlanes; // foreground , normal , background
    ...
    def addUIDrawables(objPath, drawManager, frameContext, data):

        if (DEBUG_C) print_string("glDrawDrawOverride________________________________________addUIDrawables - GET DATA");
    
        glDrawData* data = (glDrawData*)dataRaw;
        if (!data) { return; }
    
        MFnCamera cameraFn(data->cameraPath);
        MFnDagNode cameraTrsf(cameraFn.parent(0));
        MFloatMatrix camMatrix = MMatrixToMFloatMatrix(cameraTrsf.transformationMatrix());
        float camPlaneNearDistance = cameraFn.nearClippingPlane();
        float camPlaneFarDistance = cameraFn.farClippingPlane();
    
        if (DEBUG_C) print_string("glDrawDrawOverride________________________________________addUIDrawables - DRAW");
        ...
        for (int i(0); i < data->shapes.size(); i++)
        {
            ...
            int nbrElementForBuild = shapeToNbrElemForBuild[data->shapes[i]];
    
            MFloatPointArray coordsForBuild;
            MFloatVectorArray ssOffsetsForBuild;
            for (int j(0); j < nbrElementForBuild; j++)
            {
                if (data->coords.length() <= iCoords)
                {
                    exitDrawing = true;
                    break;
                }
            ...
            float distIncr = 0.001;
            float foregroundDist = camPlaneNearDistance + 0.02;
            float backgroundDist = camPlaneFarDistance - 0.02;
    
            MFloatPointArray coords;
            float size;
            float thickness;
            if ((data->drawPlaneMode == 1) && (0 < coordsForBuild.length()))//FOREGROUND
            {
            ...
            else if ((data->drawPlaneMode == 2) && (0 < coordsForBuild.length()))//BACKGROUND
            {
            ...
            MFloatVectorArray ssOffsets = ssOffsetsForBuild;
    
            //________________________________________DRAW SHAPE
    
            if (     shapesNames[data->shapes[i]] == "segment"       ) glDrawSegments(       drawManager, camMatrix, coords, ssOffsets, data->colors[i], 0, data->thicknesses[i]);
            else if (shapesNames[data->shapes[i]] == "triangle"      ) glDrawTriangles(      drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->pColors[i]);
            else if (shapesNames[data->shapes[i]] == "pointTriangle" ) glDrawPointTriangle(  drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "pointSquare"   ) glDrawPointSquare(    drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "pointCircle"   ) glDrawPointCircle(    drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "pointStar"     ) glDrawPointStar(      drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "pointCross"    ) glDrawPointCross(     drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "vector"        ) glDrawVector(         drawManager, camMatrix, coords, ssOffsets, data->colors[i], data->fills[i], size, thickness);
            else if (shapesNames[data->shapes[i]] == "text"          ) glDrawText(           drawManager, camMatrix, coords, ssOffsets, data->names[i] , data->colors[i], data->fills[i], size, thickness);     
        }
